...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //
    // CS149 Done: allocate device memory buffers on the GPU using cudaMalloc.
    //
    ...
    //
    // CS149 Done: copy input arrays to the GPU using cudaMemcpy
    //
    ...
    //
    // CS149 Done: copy result from GPU back to CPU using cudaMemcpy
    //
    ...
    double computeDuration = computeEndTime - computeStartTime;
    printf("Compute time: %.3f ms\n", computeDuration * 1000);

    //
    // CS149 Done: free memory buffers on the GPU using cudaFree
    //
